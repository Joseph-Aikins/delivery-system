...
public class GetPathFromLocation extends AsyncTask<String, Void, PolylineOptions> {
	...
	private String API_KEY = "AIzaSyAWHwZRsOBVKEB3LqET098ZTQRSAIH60oA";
	...
	protected PolylineOptions doInBackground(String... url) {
		...
		try {
			...
			try {
				...
				Utils.logger("Exception : " + e.toString());
				return null;
			} finally {
				inputStream.close();
				connection.disconnect();
			}
			Utils.logger("Background Task data : " + data);
			...
			try {
				...
				Utils.logger("Executing Routes : "/*, routes.toString()*/);
				...
				for (int i = 0; i < routes.size(); i++) {
					...
					Utils.logger("PolylineOptions Decoded");
				}
				...
				Utils.logger("Exception in Executing Routes : " + e.toString());
				return null;
			}
			
		} catch (Exception e) {
			Utils.logger("Background Task Exception : " + e.toString());
			return null;
		}
	}
